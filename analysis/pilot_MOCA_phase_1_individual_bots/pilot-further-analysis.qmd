...
##FIGURE OUT WHY IT SKIPS THESE FILES: 6qj7ucdh3r, wdszcmx4mg, anh8hw26sz, 79eufl1zzr
 ...
 osf_retrieve_node("x32pv") %>%
    ...
    osf_download(path = "data_further", conflicts = "skip")
...
#| include: false 
# 
osf_csv_filenames <- list.files(path = "data_further/")
raw_dfs_tibbles <- map(osf_csv_filenames, ~read_csv(file.path("data_further/", .)))
...
reverse_coded_mean_zero_scores_df <- reverse_coded_scores_df %>%
  filter(!grepl("slider", question)) %>%
  mutate(
    # extract the digits at the very end of the string (e.g., grabs "5" from "...:5")
    # if it's already just "5", it grabs "5". if it's pure text ("yes"), it returns NA.
    extracted_num = str_extract(answer, "\\d+$"),
    
    # do the math on the extracted number and save it as a new column
    score_centered = as.numeric(extracted_num) - 4
  ) %>%
  # drop the intermediate extraction column to keep it tidy
  select(-extracted_num)
```
...
politics_paired_df <- inner_join(
  pre_politics_df %>% select(prolific_subject_id, pre_mean_score),
  post_politics_df %>% select(prolific_subject_id, post_mean_score),
  by = "prolific_subject_id"
)

t_test_politics <- t.test(
  politics_paired_df$pre_mean_score, 
  politics_paired_df$post_mean_score, 
  paired = TRUE
)

polarization_general_paired_df <- inner_join(
  pre_polarization_general_df %>% select(prolific_subject_id, pre_mean_score),
  post_polarization_general_df %>% select(prolific_subject_id, post_mean_score),
  by = "prolific_subject_id"
)

t_test_polarization_general <- t.test(
  polarization_general_paired_df$pre_mean_score, 
  polarization_general_paired_df$post_mean_score, 
  paired = TRUE
)

polarization_specific_paired_df <- inner_join(
  pre_specific_polarization_df %>% select(prolific_subject_id, pre_mean_score),
  post_specific_polarization_df %>% select(prolific_subject_id, post_mean_score),
  by = "prolific_subject_id"
)

t_test_polarization_specific <- t.test(
  polarization_specific_paired_df$pre_mean_score, 
  polarization_specific_paired_df$post_mean_score, 
  paired = TRUE
)

sliders_paired_df <- inner_join(
  pre_sliders_df %>% select(prolific_subject_id, pre_mean_score),
  post_sliders_df %>% select(prolific_subject_id, post_mean_score),
  by = "prolific_subject_id"
)

t_test_sliders <- t.test(
  sliders_paired_df$pre_mean_score, 
  sliders_paired_df$post_mean_score, 
  paired = TRUE
)

offending_reason_paired_df <- inner_join(
  pre_offending_reason_df %>% select(prolific_subject_id, pre_mean_score),
  post_offending_reason_df %>% select(prolific_subject_id, post_mean_score),
  by = "prolific_subject_id"
)

t_test_offending_reason <- t.test(
  offending_reason_paired_df$pre_mean_score, 
  offending_reason_paired_df$post_mean_score, 
  paired = TRUE
)


openmindedness_paired_df <- inner_join(
  pre_openmindedness_df %>% select(prolific_subject_id, pre_mean_score),
  post_openmindedness_df %>% select(prolific_subject_id, post_mean_score),
  by = "prolific_subject_id"
)

t_test_openmindedness <- t.test(
  openmindedness_paired_df$pre_mean_score, 
  openmindedness_paired_df$post_mean_score, 
  paired = TRUE
)
```
...
print(t_test_polarization_specific)

merged_polarization_specific_df <- merge(pre_specific_polarization_df, post_specific_polarization_df, by = "prolific_subject_id")

polarization_specific_scatterplot <- ggplot(merged_polarization_specific_df, aes(x = pre_mean_score, y = post_mean_score)) +
  geom_point() +  # Add points
  geom_abline(intercept = 0, slope = 1, color = "darkseagreen", linetype = "dashed") +  # Line of equality +
  geom_ribbon(aes(ymin = 0, ymax = pmin(pre_mean_score, post_mean_score)), fill = "gray", alpha = 0.3) +  # Shaded area
...
print(polarization_specific_scatterplot)
...
left_join(good_data_prolific_demographics_df, by = "prolific_subject_id") %>%
  filter(!any(condition == "control free reflection")) %>%
mutate("round_of_pilot" = case_when(
    grepl("2024-07-14", `Started at.x`) ~ "Round 1 (Original prompts)",
    grepl("2024-07-17", `Started at.x`) ~ "Round 2 (Original prompts)",
    grepl("2024-07-18", `Started at.x`) ~ "Round 3 (Original prompts; conservative only)",
    grepl("2024-07-26", `Started at.x`) ~ "Round 4 (Modified prompts + combo bot)",
    TRUE ~ NA_character_
  )) %>%
...
# pivot survey demographics
survey_demographics_df <- surveys_df %>%
  filter(question %in% c('ethnicity', 'political-affiliation', 'gender-identification', 
                        'education-level', 'religious-affiliation', 'political-ideology')) %>%
...
sixteen_sub_all_demographics_df$Region <- ifelse(sixteen_sub_all_demographics_df$`Current u.s state of residence` %in% c("California (CA)", "Colorado (CO)", "Utah (UT)", "Utah(UT)", "Idaho (ID)", "Alaska (AK)", "Hawaii (HI)", "Washington (WA)", "Oregon (OR)", "Nevada (NV)", "New Mexico (NM)", "Montana (MT)", "Arizona (AZ)", "Wyoming (WY)"), "West",
                                          ifelse(sixteen_sub_all_demographics_df$`Current u.s state of residence` %in% c("Texas (TX)", "Florida (FL)", "Georgia (GA)", "North Carolina (NC)", "Tennessee (TN)", "South Carolina (SC)", "Alabama (AL)", "Mississippi (MS)", "Louisiana (LA)", "Delaware (DE)", "Maryland (MD)", "Virginia (VA)", "Washington, D.C. (DC)", "West Virginia (WV)", "Kentucky (KY)", "Arkansas (AR)", "Oklahoma (OK)"), "South",
                                                 ifelse(sixteen_sub_all_demographics_df$`Current u.s state of residence` %in% c("New York (NY)", "New Jersey (NJ)", "Massachusetts (MA)", "Pennsylvania (PA)", "Connecticut (CT)", "Rhode Island (RI)", "Vermont (VT)", "New Hampshire (NH)", "Maine (ME)"), "Northeast",
                                                        ifelse(sixteen_sub_all_demographics_df$`Current u.s state of residence` %in% c("Michigan (MI)", "Illinois (IL)", "Ohio (OH)", "Indiana (IN)", "Wisconsin (WI)", "Minnesota (MN)", "Iowa (IA)", "Missouri (MO)", "Kansas (KS)", "Nebraska (NE)", "South Dakota (SD)", "North Dakota (ND)"), "Midwest", NA))))

# define levels for categorical factors
sixteen_sub_all_demographics_df$'political-affiliation' <- fct_relevel(
  sixteen_sub_all_demographics_df$'political-affiliation',
  "Republican", "Democrat", "Independent"
)

sixteen_sub_all_demographics_df$'education-level' <- fct_relevel(
  sixteen_sub_all_demographics_df$'education-level',
  "High school diploma or GED", "Some college; no degree", "Associate degree", "Bachelor's degree", "Master's degree"
)

sixteen_sub_all_demographics_df$'political-ideology' <- fct_relevel(
  sixteen_sub_all_demographics_df$'political-ideology',
  "Very conservative", "Conservative", "Somewhat conservative", "Moderate", "Somewhat liberal", "Liberal", "Very liberal"
)

# generate summary table
demographics_table <- tbl_summary(
  sixteen_sub_all_demographics_df,
  by = NULL,
  label = list(
