...
bot_feedback <- surveys_df %>%
  ...
  select(prolific_subject_id, question, answer, condition, pilot_iteration.x) %>%
  mutate(word_count = ifelse(question == "topicChoice", NA, str_count(answer, '\\S+'))) %>%
...
#| label: fig-time-plot
#| echo: false
#| fig-cap: Scatterplot showing time spent on pre- and post-intervention surveys with color indicating whether subject was in control group or experimental group.
