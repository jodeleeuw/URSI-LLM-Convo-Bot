...
surveys_df <- surveys_cleaning_df %>%
  ...
  mutate(answer = ifelse(question == "willing-to-converse-likert-post",
                         recode(answer, 
                                ...
                                "Item 5" = "2"),
                         answer)) %>%
  # REMOVE THIS LINE WHEN ANALYZING FULL EXPERIMENT DATA. was necessary because of an oversight on my part in pilot 2 
   mutate(question = case_when(
    question == "mandating vaccines in the U.S." ~ "topic-mandating vaccines in the U.S.",
    question == "human euthanasia in the U.S." ~ "topic-human euthanasia in the U.S.",
    question == "the criminal justice system in the U.S." ~ "topic-the criminal justice system in the U.S.",
    question == "same-sex marriage in the U.S." ~ "topic-same-sex marriage in the U.S.",
    question == "the role of the U.S. government in healthcare" ~ "topic-the role of the U.S. government in healthcare",
    question == "gender equality in the U.S." ~ "topic-gender equality in the U.S.",
    question == "the atomic bombings of Hiroshima and Nagasaki" ~ "topic-the atomic bombings of Hiroshima and Nagasaki",
    TRUE ~ question 
  )
   )
```
...
```{r}
#| label: Experimening with a new way of coding Surveys data frame
#| include: false

ssurveys_cleaning_df <- raw_df %>%
  # Filter for only surveys
  filter(trial_type == "survey") %>%
  # Remove control interventions (these are coded as surveys)
  filter(phase != "control-intervention") %>%
 ...
 mutate(across(everything(), ~ gsub('"euthanasia-polarization-post":null,', '', .))) %>%
  ...
  #Add a comma before the euthanasia-slider1 question because there isn't one for some reason and it messes up the rest of the code
  ssurveys_cleaning_df$response <- gsub("\\}\"euthanasia", "\\},\"euthanasia", ssurveys_cleaning_df$response)
...
We currently have data from `r total_num_subjects_in_data` subjects in our second pilot, which we gathered on Prolific. We restricted our study to Prolific users who currently reside in the states, fluently speak English, and reported the United States as their nationality and country of birth. Per Prolific's harmful content criteria, we also restricted our study to participants who have consented to participating in studies with harmful content. Participants received a compensation of five dollars. After applying exclusion criteria, `r total_num_clean_subjects_in_data` subjects remain. We excluded `r total_failed_attention_checks` subjects for failing the two attention checks in the surveys and `r failed_copypaste_check + failed_character_count_check` subjects for likely plagiarizing their conversation messages.
