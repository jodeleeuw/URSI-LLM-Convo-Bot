...
```{r}
#| label: Reverse code relevant statements to prepare or aggregating scores for quantitative analysis
#| include: false
...
t_test_polarization <- t.test(pre_polarization_df$pre_mean_score, post_polarization_df$post_mean_score, paired = TRUE)
...
general_polarization_scatterplot <- ggplot(merged_polarization_df, aes(x = pre_mean_score, y = post_mean_score)) +
  geom_point() +  # Add points
  geom_abline(intercept = 0, slope = 1, color = "darkseagreen", linetype = "dashed") +  # Line of equality +
  geom_ribbon(aes(ymin = 0, ymax = pmin(pre_mean_score, post_mean_score)), fill = "gray", alpha = 0.3) +  # Shaded area
...
openmindedness_scatterplot <- ggplot(merged_openmindedness_df, aes(x = pre_mean_score, y = post_mean_score)) +
  geom_point() +  # Add points
  geom_abline(intercept = 0, slope = 1, color = "darkseagreen", linetype = "dashed") +  # Line of equality
  labs(x = "Pre-intervention mean rating", y = "Post-intervention mean rating", title = "Change in open-mindedness ratings before and after conversations") +
  theme_bw()

print(sliders_scatterplot)

merged_pilot1_open_df <- read_csv("opendfone copy.csv")

graph_df <- rbind(merged_openmindedness_df, merged_pilot1_open_df)

ggplot(graph_df, aes(x = pre_mean_score, y = post_mean_score)) +
  geom_point() +  # Add points
  geom_abline(intercept = 0, slope = 1, color = "darkseagreen", linetype = "dashed") +  # Line of equality
  labs(x = "Pre-intervention mean rating", y = "Post-intervention mean rating", title = "Change in open-mindedness score") +
  theme_bw()
...
```{r}
#| label: Create df with just convo transcripts 
#| echo: false
...
## Combo bot (chain prompting) parse

```{r}
#| label: Parse the combo bot transcripts for readability 
#| echo: false
...
```{r}
#| label: Parse selection bot (response election format)
#| echo: false
...
# make demographics df. need demo data so we can define the round that the people were in (so we know whether they chatted with an older bot)
...
bot_feedback <- surveys_df %>%
  ...
  select(prolific_subject_id, question, answer, condition, pilot_iteration)
...
character_count_flagging <- raw_df %>%
  ...
  #remove rows that aren't keypress logs
  filter(!grepl('^\\[\\{"role":"system","content":', logs)) %>%
  #make a new column for the person's actual output
   mutate(logs = gsub('"role":"assistant","content".*', '', logs)) %>%
  #remove some excess punctuation
  mutate(logs = gsub('":\\[', '', logs)) %>%
  mutate(logs = gsub('\\]\\},\\{', '', logs)) %>%
...
#also look at number of individual key presses and number of characters in responses. anytime response is longer than keypresses should be flagged 
