...
# name the round of the pilot that the person was in 
  mutate(pilot_iteration = case_when(
    grepl(c("2025-07-10|2025-07-08"), start) ~ "Willingness Survey Pretest",
    TRUE ~ NA_character_
  )) 
```
...
files_filtered <- files %>%
  ...
  filter(date_created %in% as_date(c("2025-07-10", "2025-07-08")))
...
# adding subject id's into df 
rawwwwwww_df <- rawwwwwww_df %>%
mutate(
  prolific_subject_id = if_else(row_number() > 4, 1234, prolific_subject_id),
  prolific_subject_id = if_else(row_number() < 4, 5678, prolific_subject_id)
  )
...
surveys_cleaning_df <- raw_df %>%
  ...
  # openmindedness and offending reason
  mutate(across(everything(), ~ gsub(',"openmindedness":null,', '', .))) %>%
  mutate(across(everything(), ~ gsub('"openmindedness":', '', .))) %>%
  mutate(across(everything(), ~ gsub(',"offending-reason":null,', '', .))) %>%
  mutate(across(everything(), ~ gsub('"offending-reason":', '', .))) %>%
...
surveys_df <- surveys_cleaning_df %>%
  ...
  # Bin the religious affiliations because they were free response 
   mutate(answer = case_when(
     question == "religious-affiliation" & str_detect(str_to_lower(answer), "christian|catholic|christianity") ~ "Christian/Catholic",
...
attention_check <- surveys_df %>%
  ...
  mutate(flag = case_when(
    sum(answer == "0.00") == 0 ~ "fail (single)", #double fail means we should reject their data
    sum(answer == "0.00") == 1 ~ "pass", #good subject :3
    TRUE ~ NA_character_
  )) %>%
  ungroup() %>%
  select(flag, question, answer, prolific_subject_id, time_taken_overall_minutes) 
...
demo_table_df$Region <- case_when(
  demo_table_df$`Current u.s state of residence` %in% c("California (CA)", "Colorado (CO)", "Utah (UT)", "Utah(UT)", "Idaho (ID)", "Alaska (AK)", "Hawaii (HI)", "Washington (WA)", "Oregon (OR)", "Nevada (NV)", "New Mexico (NM)", "Montana (MT)", "Arizona (AZ)", "Wyoming (WY)") ~ "West",
  demo_table_df$`Current u.s state of residence` %in% c("Texas (TX)", "Florida (FL)", "Georgia (GA)", "North Carolina (NC)", "Tennessee (TN)", "South Carolina (SC)", "Alabama (AL)", "Mississippi (MS)", "Louisiana (LA)", "Delaware (DE)", "Maryland (MD)", "Virginia (VA)", "Washington, D.C. (DC)", "West Virginia (WV)", "Kentucky (KY)", "Arkansas (AR)", "Oklahoma (OK)") ~ "South",
   demo_table_df$`Current u.s state of residence` %in% c("New York (NY)", "New Jersey (NJ)", "Massachusetts (MA)", "Pennsylvania (PA)", "Connecticut (CT)", "Rhode Island (RI)", "Vermont (VT)", "New Hampshire (NH)", "Maine (ME)") ~ "Northeast",
  demo_table_df$`Current u.s state of residence` %in% c("Michigan (MI)", "Illinois (IL)", "Ohio (OH)", "Indiana (IN)", "Wisconsin (WI)", "Minnesota (MN)", "Iowa (IA)", "Missouri (MO)", "Kansas (KS)", "Nebraska (NE)", "South Dakota (SD)", "North Dakota (ND)") ~ "Midwest",
 TRUE ~ NA_character_
)
...
offending_reasons_names <- colnames(wide_for_likert_df)
### create 7 new rows
...
wide_for_likert_df <- bind_rows(wide_for_likert_df, new_likert_rows)



# standardize all columns to use the same factor object
wide_for_likert_df <- wide_for_likert_df %>%
  mutate(across(everything(), ~ factor(.x, levels = all_likert_values, ordered = TRUE)))

offending_reason_likert <- likert(as.data.frame(wide_for_likert_df))
...
wide_for_likert_openmindedness_df <- openmindedness_df %>%
  select(question, answer, prolific_subject_id) %>%
  pivot_wider(names_from = question, values_from = answer) %>%
  ungroup() %>%
...
```{r}
#| label: tbl-bot-free-response-examples-table
#| echo: false
#| tbl-cap: Table showing examples of subjects' responses to free response question, which asked subjects if there were any concerns beyond the nineteen items in our survey that contributed to the subject's unwillingness.
...
```{r}
#| label: tbl-bot-free-response-tally-plot
#| echo: false
...
```{r}
#| label: Correlations open-mindedness vs. willingness concerns 
#| echo: false
#| tbl-cap: Correlation matrix of open-mindedness vs. willingness concerns 

# look at correlation between open-mindedness score and each willingness concern
cor_results <- concerns_labeled_wide %>%
  map_dfr(~ tidy(cor.test(.x, survey_df$open-mindedness_score)), .id = "item")
...
apply(concerns_labeled_wide, 2, function(x) cor(x, survey_df$slider_avg, use = "pairwise.complete.obs"))
...
model <- lm(willingness_avg ~ ., data = cbind(survey_df["slider_avg"], concerns_labeled_wide))
summary(model)
...
manova_result <- manova(as.matrix(concerns_labeled_wide) ~ survey_df$topic)
