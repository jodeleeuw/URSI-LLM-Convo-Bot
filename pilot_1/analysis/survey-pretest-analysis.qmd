...
We currently have data from `r total_num_subjects_in_data` subjects in our willingness survey questions pretest experiment, which we gathered on Prolific. To remain consistent with the demographic that our main experiment will target, we restricted our study to Prolific users who currently reside in the states, fluently speak English, and reported the United States as their nationality and country of birth. Per Prolific's harmful content criteria, we also restricted our study to participants who have consented to participating in studies with harmful content. Participants received a compensation of one dollar. We excluded `r total_failed_attention_checks` subjects for failing one or more of the attention checks. After applying this exclusion criteria, `r total_num_clean_subjects_in_data` subjects remain. 
...
```{r}
#| label: calculating average times
#| echo: false
