...
raw_df <- rawwwwwww_df %>%
  ...
  left_join(cleaned_prolific_demographics_df, by = "prolific_subject_id") %>%
...
```{r}
#| label: WIP - KMO & Bartlett's test on 19-item matrix
#| echo: false
...
concerns_labeled_wide_with_sub_labels <- surveys_df_labeled_questions %>%
  select(prolific_subject_id.x, short_label, answer) %>%
...
concerns_labeled_wide <- concerns_labeled_wide_with_sub_labels %>%
  select(-prolific_subject_id.x)

# KMO and Bartlett’s test
#kmo <- KMO(concerns_labeled_wide)
#cortest_bartlett <- cortest.bartlett(cor(concerns_labeled_wide), n = nrow(concerns_labeled_wide))

#NEED TO SEE WHETHER KMO AND COTEST WORK W ENOUGH DATA
```
...
#fa_parallel <- fa.parallel(concerns_labeled_wide, fa = "fa")
...
#cor_openmindedness_mean_vs_concerns <- concerns_labeled_wide %>%
#   map_dfr(~ tidy(cor.test(.x, openmindedness_df_mean$mean_openmindedness)), .id = "concern")
...
#   select(reason_item, estimate, p.value) %>%
#   arrange(p.value)
