...
```{r}
#| label: WIP - Correlations open-mindedness individual scores vs. willingness concerns 
#| echo: false
#| tbl-cap: Correlation matrix of open-mindedness vs. willingness concerns 

# create merged df 
openmindedness_and_willingness_merged_df <- merge(wide_for_likert_offending_reasons_df_with_sub_labels, wide_for_likert_openmindedness_df_with_sub_labels, by = 'prolific_subject_id') %>%
  ungroup() %>%
  select(-prolific_subject_id)

# look at correlation between each open-mindedness score and each willingness concern
cor(openmindedness_and_willingness_merged_df, use = "pairwise.complete.obs")
...
reasons_and_willingness_merged_df <- merge(wide_for_likert_offending_reasons_df_with_sub_labels, sliders_df, by = 'prolific_subject_id') %>%
  ungroup() %>%
  select(-prolific_subject_id)
