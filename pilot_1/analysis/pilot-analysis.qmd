...
```{r}
#| label: Read in demographics csv and merge it with the raw df based on subject id
#| include: false

#read in data

#raw_demographics_tibbles <- map(demographics_csv_filenames, ~read_csv(file.path("data/", .)))
#raw_df <- bind_rows(raw_demographics_tibbles)

#demographics_df <- read_csv("prolific_export_6690246c6dba4b8beab3c615.csv") %>%
#read_csv("prolific_export_6690246c6dba4b8beab3c615.csv") %>%
#  read_csv("prolific_export_6690246c6dba4b8beab3c615.csv") %>%
 # read_csv("prolific_export_6690246c6dba4b8beab3c615.csv") %>%
#add a merge 

#rename subject id column to match our name so that we can merge by it
#names(demographics_df)[names(demographics_df) == 'Participant id'] <- 'prolific_subject_id'

#merge
#full_raw_df <- raw_df %>%
 # full_join(demographics_df, by = "prolific_subject_id")
```
...
surveys_cleaning_df <- raw_df %>%
  # Remove the first four subjects of the pilot who participant in the experiment before Jackie made the code easier to perform data analysis on 
...
sixteen_demographics_df <- prolific_demographics_df %>%
  left_join(survey_demographics_df, by = "prolific_subject_id", suffix = c("_prolific", ""))

sixteen_demographics_df <- sixteen_demographics_df
